...
{
    public partial class EditProject : SimplePage
    {
        private readonly ILog _log = LogManager.GetLogger(typeof(EditProject));

        public EditProject() : base(Web.PageExtensions.CustomPageLink.OptionFlag, 0) { }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                targetLanguages.DataSource = EPLangUtil.GetValidLanaguages();
                targetLanguages.DataTextField = "Name";
                targetLanguages.DataValueField = "Locale";
                targetLanguages.DataBind();
                ListItem li = targetLanguages.Items.FindByValue(EPLangUtil.GetValidLanguageString(sourceLanguage.SelectedValue));
            ...
            else
            {
                ...
                _log.Debug(String.Format("Last step is: {0}", laststep));
        ...
        protected void WizardFinished(object sender, EventArgs e)
        {
            ...
            string sessionID = ViewState[Constants.VIEWSTATSESSIONID] as string;
            const TranslationStatus status = TranslationStatus.Created;
            ...
            TranslationProject tproj = Manager.Current.GetTranslationProject(projectid);
            ...
            foreach (ListItem item in targetLanguages.Items)
            {
                if (item.Selected)
                {
                    ...
                    foreach (TranslationPage tp in tproj.Pages)
                    {
        ...
        private bool IsPageLoaded
        {
            get
            {
                string ret = ViewState[Constants.VIEWSTATISPAGELOADED] as string;
                if (ret == null)
                {
                    return false;
                }
