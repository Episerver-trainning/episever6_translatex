...
namespace EPiServer.Research.Connector.Language.LionBridge
{
    public class Connector : IConnector
    {
        ...
        private Vojo _vojo = null;
        private FreewayAuth.FreewayAuth _auth = null;
        private string wsurl = ApplicationSettings.Instance.TranslationWs; //ConfigurationManager.AppSettings["translationws"];
        string wsauthurl = ApplicationSettings.Instance.TranslationAuthenticationWs; //ConfigurationManager.AppSettings["translationauthws"];
        private string username = ApplicationSettings.Instance.TranslationUserName; //ConfigurationManager.AppSettings["translationuser"];
        string password = ApplicationSettings.Instance.TranslationPassword; //ConfigurationManager.AppSettings["translationpwd"];
            ...
            foreach (TranslationFile tf in p.Files)
            {
                ...
                foreach (string lang in p.TargetLanguages)
                {
                    if (tf.GetStatus(lang)==TranslationStatus.Created)
                    {
                        ...
                        foreach (FreewayWS.FileInfo fi in fil.FileInfoItems)
                        {
